...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Olá! Sou o assistente da SI Soluções Imobiliárias. Como posso ajudar com seus leads hoje? 🏠' },
  ]);
      ...
      <div className={cn(
          ...
          {messages.map((msg, i) => (
            <div key={i} className={cn('flex items-start gap-2', msg.role === 'user' && 'flex-row-reverse')}>
              <div className={cn(
                ...
                msg.role === 'assistant' ? 'bg-brand-100 dark:bg-brand-900/30' : 'bg-slate-100 dark:bg-slate-700'
              )}>
                {msg.role === 'assistant'
        ...
        {messages.length === 1 && (
          <div className="px-4 pb-2 flex flex-wrap gap-1">
            {['Dicas para qualificar leads', 'Como fechar mais negócios?', 'Qual status devo priorizar?'].map((s) => (
