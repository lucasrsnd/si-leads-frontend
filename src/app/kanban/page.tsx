...
function KanbanCard({ lead, onClick }: { lead: Lead; onClick: () => void }) {
  ...
  const style: React.CSSProperties = {
    ...
    boxShadow: isDragging ? '0 20px 40px rgba(99,102,241,0.25)' : undefined,
  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      onClick={onClick}
      className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-4 hover:border-indigo-400/50 hover:shadow-md transition-colors duration-150 relative overflow-hidden select-none">
      <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl" style={{ background: COLUMN_COLORS[lead.status] }} />
      <div className="mb-3">
        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold"
          style={{ background: `${COLUMN_COLORS[lead.status]}20`, color: COLUMN_COLORS[lead.status] }}>
          {lead.name.split(' ').slice(0, 2).map((n: string) => n[0]).join('').toUpperCase()}
        </div>
      </div>
      <p className="text-sm font-semibold text-[var(--text)] leading-snug mb-1">{lead.name}</p>
      {lead.phone && <p className="text-xs text-[var(--text-muted)]">{lead.phone}</p>}
      <div className="flex items-center justify-between mt-3">
        <span className={cn('text-xs font-semibold', PRIORITY_COLORS[lead.priority])}>● {lead.priority}</span>
        {lead.source && (
...
function KanbanColumn({ col, leads, onCardClick }: { col: LeadStatus; leads: Lead[]; onCardClick: (id: string) => void }) {
  const { setNodeRef, isOver } = useDroppable({ id: col });

  return (
    <div className="flex-shrink-0 w-64 flex flex-col">
      <div className="flex items-center gap-2 mb-3 px-1">
        <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: COLUMN_COLORS[col] }} />
        <span className="text-sm font-semibold text-[var(--text)]">{STATUS_LABELS[col]}</span>
      ...
      <div ref={setNodeRef}
        className={cn(
          'flex-1 min-h-40 space-y-2.5 rounded-2xl p-2.5 border-2 transition-all duration-150',
          isOver
        ...
        {leads.length === 0 && (
          <div className={cn(
            ...
            isOver ? 'border-indigo-400 text-indigo-500' : 'border-[var(--border)] text-[var(--text-muted)]'
          )}>
...
export default function KanbanPage() {
  ...
  const totalVisible = filteredBoard ? Object.values(filteredBoard).flat().length : 0;
      ...
      <div className="space-y-5 animate-fade-up">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[var(--text-muted)] text-sm mt-0.5">{totalVisible} leads · clique para detalhes, arraste para mover</p>
          </div>
